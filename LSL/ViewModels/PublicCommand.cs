    ...
    public class PublicCommand : RegionalVMBase
    {
        ...
        private async Task OpenWebPage(string url)
        {
            ...
            catch (Win32Exception noBrowser)
            {
                ...
                {
                    await AppState.ITAUnits.ThrowError("打开网页失败",
        ...
        private async Task OpenExplorer(string url)
        {
            ...
            {
                _logger.LogError(ae, "File or directory not found when trying to open file explorer.");
                await AppState.ITAUnits.ThrowError("打开文件失败", $"不存在位于{url}的文件或目录。");
            }
            catch (DirectoryNotFoundException dnfe)
            {
                _logger.LogError(dnfe, "Parent directory not found when trying to open file explorer.");
                await AppState.ITAUnits.ThrowError("打开文件失败", $"无法获取位于{url}的文件的父目录。");
            }
            catch (Exception e)
            {
                _logger.LogError(e, "Error executing open explorer task.");
                await AppState.ITAUnits.ThrowError("打开文件失败", $"在文件资源管理器中打开{url}时出现以下报错：\r{e.Message}");
            }
        }
