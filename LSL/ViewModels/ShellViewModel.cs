...
public partial class ShellViewModel : ViewModelBase
{
    ...
    ) : base(appState.LoggerFactory.CreateLogger<ShellViewModel>())
    {
        ...
        FullViewBackCmd = ReactiveCommand.Create(() => MessageBus.Current.SendMessage(new NavigateArgs
        {
            BarTarget = BarState.Common, LeftTarget = GeneralPageState.Undefined, RightTarget = RightPageState.Undefined
        }));

        #region 多参数导航

        ServerConfigCmd = ReactiveCommand.CreateFromTask(async () =>
        ...
        MessageBus.Current.Listen<WindowOperationArgs>()
            ...
            .Subscribe(
                result =>
                {
                    ...
                    {
                        case 1:
                            MessageBus.Current.SendMessage(new WindowOperationArgs(WindowOperationArgType.Hide)); break;
                        case 0:
                            MessageBus.Current.SendMessage(
                                new WindowOperationArgs(WindowOperationArgType.ConfirmClose)); break;
                    }
                },
                ex =>
                {
                    Logger.LogError(ex, "An error occured while processing window exit operation.");
                    Coordinator.PopupInteraction.Handle(new InvokePopupArgs(PopupType.ErrorConfirm, "窗口退出处理错误",
                        $"LSL在处理退出操作时出现了错误。\n{ex}"));
                }
            );
    }
    ...
    private async Task<int> PreExitOperations() // 退出事件处理
    {
        ...
        var daemon = AppState.CurrentConfigs.TryGetValue("daemon", out var daemonObj) &&
                     bool.TryParse(daemonObj.ToString(), out var daemonConf) && daemonConf;
        if (daemon) return 1;
