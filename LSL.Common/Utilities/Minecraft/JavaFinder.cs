...
public static class JavaFinder
{
    ...
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) return GetWindowsJavaInfos();

        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) return GetLinuxJavaInfos();

        if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX)) return GetMacOSJavaInfos();
    ...
    private static async Task<List<JavaInfo>> GetWindowsJavaInfos() // 获取Windows系统中的Java信息
    {
        ...
        {
            var javaPath = Path.Combine(pathPart, "java.exe");
            var javawPath = Path.Combine(pathPart, "javaw.exe");
            if (File.Exists(javaPath))
            {
                var javaInfo = await GetJavaInfo(javaPath);
                if (javaInfo != null) javaInfos.Add(javaInfo);
            }
    ...
    private static async Task<List<JavaInfo>> GetLinuxJavaInfos() // 获取Linux系统中的Java信息
    {
        ...
        foreach (var jvmDir in jvmDirs)
        {
            ...
            await Parallel.ForEachAsync(Directory.GetDirectories(jvmDir), ConcurrencyOptions.ConcurrencyLimit,
                ...
                {
                    // 查找java可执行文件
                    var javaPath = Path.Combine(subDir, "bin", "java");
                    if (File.Exists(javaPath))
                    {
                        var javaInfo = await GetJavaInfo(javaPath);
                        if (javaInfo != null) javaInfos.Add(javaInfo);
    ...
    private static async Task<List<JavaInfo>> GetMacOSJavaInfos() // 获取MacOS系统中的Java信息
    {
        ...
        if (Directory.Exists(jvmDir))
            await Parallel.ForEachAsync(Directory.GetDirectories(jvmDir), ConcurrencyOptions.ConcurrencyLimit,
                ...
                {
                    // 查找java可执行文件
                    var javaPath = Path.Combine(subDir, "Contents", "Home", "bin", "java");
                    if (File.Exists(javaPath))
                    {
                        var javaInfo = await GetJavaInfo(javaPath);
                        if (javaInfo != null) javaInfos.Add(javaInfo);
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(output))
            {
                ...
                if (lines.Length > 1)
                {
                    ...
                    // get vendor
                    var vendorLine = lines[1];
                    var vendorParts = vendorLine.Split(' ');
                    if (vendorParts.Length >= 3)
                    {
                        if (vendorLine.Contains("Java(TM) SE Runtime Environment")) vendor = "Oracle";
                        if (vendorLine.Contains("OpenJDK Runtime Environment")) vendor = "OpenJDK";
                        if (vendorLine.Contains("Temurin")) vendor = "Adoptium";
                        if (vendorLine.Contains("Corretto")) vendor = "Corretto";
                        if (vendorLine.Contains("IBM")) vendor = "IBM";
                        if (vendorLine.Contains("Microsoft")) vendor = "Microsoft";
                        if (vendorLine.Contains("Zulu")) vendor = "Zulu";
                    }

                    // get version
                    var versionLine = lines[0];
                    var versionParts = versionLine.Split(' ');
                    if (versionParts.Length >= 3) version = versionParts[2].Trim('"');
                    if (vendor == "Oracle" && version.StartsWith("1.")) version = version[2..];
                    // get architecture
                    var architectureLine = lines[2];
