...
public static class ForgeConfigHelper
{
    ...
    public static async Task<ServiceResult<ForgeCoreConfigV1>> GetForgeConfig(string? serverDir)
    {
        if (string.IsNullOrWhiteSpace(serverDir) || !Directory.Exists(serverDir))
            return ServiceResult.Fail<ForgeCoreConfigV1>(new DirectoryNotFoundException("Server directory doesn't exist."));
    ...
    /// <returns>Forge config.</returns>
    private static async Task<ServiceResult<ForgeCoreConfigV1>> FindLibraryArgsWithScript(string unixScriptPath, string winScriptPath)
    {
        ...
        if (unixArgs is null && winArgs is not null) return ServiceResult.Success(new ForgeCoreConfigV1(winArgs.Replace("win_args.txt", "unix_args.txt"), winArgs));
        if (unixArgs is not null && winArgs is null) return ServiceResult.Success(new ForgeCoreConfigV1(unixArgs, unixArgs.Replace("unix_args.txt", "win_args.txt")));
        return ServiceResult.Fail<ForgeCoreConfigV1>("Cannot find any of the forge libraries through scripts.");
    }
    ...
    /// <returns>Forge config.</returns>
    private static ServiceResult<ForgeCoreConfigV1> FindLibraryArgsWithContrast(string serverPath)
    {
        if (string.IsNullOrWhiteSpace(serverPath) || !File.Exists(serverPath)) return ServiceResult.Fail<ForgeCoreConfigV1>(
        ...
        if (!Directory.Exists(dir)) return ServiceResult.Fail<ForgeCoreConfigV1>("Server forge directory doesn't exist.");
        var subDirs = Directory.GetDirectories(dir);
        if (subDirs.Length != 1) return ServiceResult.Fail<ForgeCoreConfigV1>("Cannot determine the directory of which library arguments existed");
        var subDir = Path.GetDirectoryName(subDirs[0]);
        return string.IsNullOrWhiteSpace(subDir)
            ? ServiceResult.Fail<ForgeCoreConfigV1>("Cannot determine the directory of which library arguments existed")
            : ServiceResult.Success(new ForgeCoreConfigV1($"libraries/net/minecraftforge/forge/{subDir}/unix_args.txt",
