...
public static class ForgeConfigHelper
{
    ...
    /// <returns>Forge config.</returns>
    private static async Task<Result<ForgeCoreConfigV1>> FindLibraryArgsWithScript(string unixScriptPath, string winScriptPath)
    {
        ...
        if (unixArgs is null && winArgs is not null) return Result.Success(new ForgeCoreConfigV1(winArgs.Replace("win_args.txt", "unix_args.txt"), winArgs));
        if (unixArgs is not null && winArgs is null) return Result.Success(new ForgeCoreConfigV1(unixArgs, unixArgs.Replace("unix_args.txt", "win_args.txt")));
    ...
    {
        if (string.IsNullOrWhiteSpace(serverPath) || !File.Exists(serverPath)) return Result.Fail<ForgeCoreConfigV1>(
        ...
        if (subDirs.Length != 1) return Result.Fail<ForgeCoreConfigV1>("Cannot determine the directory of which library arguments existed");
        ...
        return string.IsNullOrWhiteSpace(subDir)
            ...
            : Result.Success(new ForgeCoreConfigV1($"libraries/net/minecraftforge/forge/{subDir}/unix_args.txt",
