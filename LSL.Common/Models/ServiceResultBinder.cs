...
public static class ServiceResultBinder
{
        ...
        Func<T, Task<ServiceResult<TResult>>> binder)
    {
        ...
        if (result.IsWarning)
        {
            ...
            if (nextResult.IsWarning) return ServiceResult.Warning(nextResult.Result, new AggregateException(result.Error!, nextResult.Error));
        ...
        Func<T, Task<ServiceResult>> binder)
    {
        ...
        if (result.IsWarning)
        {
            ...
            if (nextResult.IsWarning) return ServiceResult.Warning(new AggregateException(result.Error!, nextResult.Error));
